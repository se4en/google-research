...
def evaluate(model, test_ratings, test_negatives, K=10):
    ...
    (hits, ndcgs) = evaluate_model(
        model, test_ratings, test_negatives, K=K, num_thread=1
    )
...
def main():
    ...
    parser.add_argument(
        "--data", type=str, default="Data/ml-1m", help="Path to the dataset"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Embedding dimensions, the first dimension will be " "used for the bias.",
    )
    parser.add_argument(
        "--regularization",
        type=float,
